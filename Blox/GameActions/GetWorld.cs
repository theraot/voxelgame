...
namespace Hexpoint.Blox.GameActions
{
	internal class GetWorld : GameAction
	{
		...
		{
			if (!Facade.Configuration.IsServer) throw new Exception("Only the server needs this");
			Immediate = true;
			...
			{
				using (var gzstream = new DeflateStream(memstream, CompressionMode.Compress, true))
				{
					...
					using (var buffstream = new BufferedStream(gzstream, 65536))
					{
		...
		internal override void Receive()
		{
			lock (TcpClient)
			{
				...
				{
					using (var gzstream = new DeflateStream(memstream, CompressionMode.Decompress))
					{
